...
const form = ref({ nom_patient: '', description: '', niveau: 'urgent' });
...
const creer = async () => {
  ...
  try {
    ...
    form.value = { nom_patient: '', description: '', niveau: 'urgent' };
  ...
  <AppLayout>
    <div class="space-y-5">
      <div class="flex items-center justify-between">
        ...
        <button @click="showModal = true" class="btn btn-danger">🚨 Signaler urgence</button>
      </div>
      <div class="space-y-3">
        <div v-if="loading" class="card py-8 text-center text-gray-400">Chargement...</div>
        <div v-for="u in urgences" :key="u.id_urgence"
          class="card border-l-4"
          :class="{ 'border-l-red-500': ['critique','urgent'].includes(u.niveau), 'border-l-yellow-500': u.niveau === 'modere', 'border-l-green-500': u.niveau === 'faible' }">
          <div class="flex items-center gap-3">
            <div class="flex-1">
              ...
              <p class="text-sm text-gray-600 mt-1">{{ u.description }}</p>
              <p class="text-xs text-gray-400">{{ new Date(u.date_arrivee).toLocaleString('fr') }}</p>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
        ...
        <form @submit.prevent="creer" class="space-y-3">
          ...
          <div class="flex gap-2 pt-2">
            ...
            <button type="submit" :disabled="saving" class="btn btn-danger flex-1">Signaler</button>
          </div>
        </form>
      </div>
    </div>
